...
test('pan and zoom survive expansion while restore returns to the full normal view', async ({
  ...
  await expect
    .poll(async () => {
      const panned = await readMapView(page);
      return (
        Math.abs(panned.center[0] - zoomed.center[0]) +

...
for (const viewport of PORTRAIT_VIEWPORTS) {
  test(
    `keeps the map first and controls compact at ${viewport.width}px`,
    async ({ page }, testInfo) => {
...
test('expands one mobile control without sacrificing width or accessibility', async ({ page }) => {
  ...
  await expect(
    page.getByRole('list', { name: 'Resultados de búsqueda de lugares' }),
  ).toBeVisible();
  await expectNoHorizontalOverflow(page);
...
test(
  'runs the MAP-033 suite with Android Chromium and iOS WebKit emulation',
  async ({ page }, testInfo) => {

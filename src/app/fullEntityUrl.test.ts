...
describe('full entity URL contract', () => {
  it('creates a canonical query-only URL under the existing pathname', () => {
    const source = new URL(
      'https://example.test/castigo-divino-map/?place=demo&q=guard&category=settlement&tag=watch#map',
    );
  ...
  it('parses and canonicalizes valid entity URLs without preserving map dimensions', () => {
    const source = new URL(
      'https://example.test/castigo-divino-map/?entity=harbor-guard&q=ignored&extra=value#ignored',
    );

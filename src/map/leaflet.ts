...
export function mountFaerunMap(
  ...
  const schedulePortrait = (
    ...
    portraitQueue.push(async () => {
      ...
      try {
        ...
        if (!url || request.signal.aborted || destroyed || generation !== portraitGeneration) return;
        const element = leafletMarker.getElement();
  ...
  collapseSpiderfy = (restoreFocus = false): void => {
    ...
    if (restoreFocus && clusterElement?.isConnected) {
      window.requestAnimationFrame(() => {
        if (clusterElement.isConnected) clusterElement.focus({ preventScroll: true });
      });
    }
  };
  ...
  const locatePin = (pinId: string): void => {
    ...
    map.setView(L.latLng(pin.coordinate[0], pin.coordinate[1]), targetZoom, { animate: false });
    map.panInsideBounds(bounds, { animate: false });

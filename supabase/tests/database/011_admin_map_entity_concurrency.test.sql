...
select set_config(
  ...
  public.admin_get_map_entity_editor('entity-map019-stale-row')->'record'->>'updated_at',
  true
);
select set_config(
...
set summary = 'changed by a concurrent editor'
where id = 'entity-map019-stale-row';
...
select throws_ok(
  $$select public.admin_save_map_entity(
      ...
      'map019-stale-row', 'character', 'pin', 'Stale row', '', '', 500, 500,
      'category-people', 'draft', '{}'::text[],
      '[{"player_id":"player-demo-one","disposition":"neutral"},{"player_id":"player-demo-two","disposition":"neutral"}]'::jsonb
    )$$,
  '40001',
  'the entity changed while it was being edited',
  'a stale updated_at blocks a concurrent overwrite of the same entity'
);

select is(
  (select summary from public.map_entities where id = 'entity-map019-stale-row'),
  'changed by a concurrent editor',
  'the rejected stale save preserves the winning concurrent update'
);

...
-- MAP-060 compatibility hardening: v6 acquires the per-entity advisory lock
-- before taking a map_entities row lock. Legacy v5 must use the same order;
-- otherwise a v5 writer can hold the row while waiting for v4's advisory lock
-- as a concurrent v6 writer holds that advisory lock while waiting for the row.
...
begin
  ...
  -- Match v4/v6 before any row-level lock. v4 will reacquire the same
  -- transaction-scoped advisory lock later; PostgreSQL advisory locks are
  -- re-entrant for the owning session, so that nested call remains safe.

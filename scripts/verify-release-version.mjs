...
const renderApp = readFileSync(
  new URL('../src/app/renderApp.ts', import.meta.url),
  'utf8',
);
...
const projectStatus = readFileSync(
  new URL('../docs/project-status.md', import.meta.url),
  'utf8',
);
...
assert(
  renderApp.includes(`Faerûn · ${expectedLabel}`),
  `UI eyebrow must declare ${expectedLabel}`,
);
assert(
...
assert(
  pagesSmoke.includes(expectedLabel),
  'Pages smoke must assert the current release label',
);
assert(
...
console.log(
  `MAP-066 release version verification passed: ${expectedVersion} / ${expectedLabel}.`,
);
